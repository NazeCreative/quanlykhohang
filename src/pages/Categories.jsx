...
const Categories = () => {
  ...
  const handleFinish = async (values) => {
    try {
      if (editingCategory) {
        await updateDoc(doc(db, 'categories', editingCategory.id), { name: values.name });
        message.success('Cập nhật thành công!');
      } else {
        await addDoc(collection(db, 'categories'), { name: values.name });
  ...
  return (
    <div>
      ...
      <Table columns={columns} dataSource={categories} loading={loading} bordered />
